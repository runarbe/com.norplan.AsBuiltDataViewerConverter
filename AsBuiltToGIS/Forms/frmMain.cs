    ...
    public partial class frmMain : Form
    {
        ...
        private void parseToGMLToolStripMenuItem_Click_1(object sender, EventArgs e)
        {
            if (frmMain.dbx != null)
            {
                ...
                {
                    Utilities.LogDebug("Aborted export to GML...");
                    return;
                }
                var m = ExtFunctions.ExportSelectedDatabaseToGML(dlgSaveFile.FileName);
        ...
        {
            theMap.MapFrame.SuspendEvents();
            frmMain.lyrBingSatellite = BruTileLayer.CreateBingAerialLayer();
            this.theMap.Layers.Add(frmMain.lyrBingSatellite);
            this.theMap.MapFrame.ResumeEvents();
        ...
        private void testQRCodesOfSelectedLayerToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                dlgSaveFile.FileName = DateTime.Now.ToString("yyyyMMdd") + "-qrtest.log";
                dlgSaveFile.Title = "Please select a log file location (optional)";
                dlgSaveFile.Filter = "*.log|Log files";
                if (dlgSaveFile.ShowDialog() != System.Windows.Forms.DialogResult.OK)
                {
                ...
                else
                {
                    foreach (var mFeature in mLayer.DataSet.Features)
                    {
                        ...
                        if (mFeature.FeatureType == FeatureType.Point)
                        {
                            ...
                            Log(mQRCode.TestQRCode(mDistrictsShapefile, false, mPoint.X, mPoint.Y));
                        }
                        else
                        {
                            Log(mQRCode.TestQRCode(mDistrictsShapefile));
                        }
                        Application.DoEvents();
                    }
                }
